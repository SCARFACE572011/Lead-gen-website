...
--
--   1. pipeline              CRM pipeline columns on saved leads
--   2. audit_reports         shareable audit report table (anon has no access)
--   3. daily_search_counter  tamper-proof counter for the fair-use cap
--   4. admin_allowlist       moves auto-admin emails out of the signup trigger
--   5. saved_search_country  keeps country and km on saved searches
--   6. gclid                 ad click id on the profile, for attribution
--   7. lock_usage_counters   stops users editing their own usage counters
--
-- Order matters: lock_usage_counters runs last because it locks down tables the
-- earlier steps still need to write.
-- ============================================================================
